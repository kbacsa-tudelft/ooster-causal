...
def main():
    ...
    base_overrides = {k: v for k, v in vars(args).items() if k not in ('sweep', 'sweep_root', 'dry_run')}
    ...
    for i, combo in enumerate(combos, 1):
        ...
        results.append({'name': name, 'ok': ok, 'time_s': elapsed, 'log': log_path})

    print('\n' + '=' * 78)
    print(f'{"run":50s} {"ok":5s} {"time(s)":>9s}')
    for r in results:
        print(f'{r["name"]:50s} {"yes" if r["ok"] else "NO":5s} {r["time_s"]:9.1f}')
